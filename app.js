...
const ACTIONS=[
  ...
  {id:"blockade",icon:"封",name:"断其财路",desc:"堵住相邻敌方地盘的货路与水路，让那堵墙自己往下掉。",effects:["目标3个月不长驻防","每月驻防-10"],max:1,canRun:s=>{const id=blockadeTarget(s);return !!id&&s.cash>=blockadeCost(s,id)},lockedText:s=>blockadeTarget(s)?"现金不足":"没有可封锁的相邻地盘",run:s=>{runBlockade(s,blockadeTarget(s))}},
  {id:"turncoat",icon:"策",name:"重金策反",desc:"不砸那扇门，买下门后面那个人。成了对方驻防塌一角，砸了这笔钱就是替对方点的火。",effects:["成功 驻防-25","失败 声望-3"],max:1,canRun:s=>{const id=turncoatTarget(s);return !!id&&s.cash>=turncoatCost(s,id)},lockedText:s=>turncoatTarget(s)?"现金不足":"没有可策反的相邻敌方地盘",run:(s,rng=Math.random)=>{runTurncoat(s,turncoatTarget(s),rng)}}
];
