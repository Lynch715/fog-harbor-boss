...
const ACTIONS=[
  ...
  {id:"intel",icon:"眼",name:"打听敌情",desc:"查清一块相邻地盘的真实驻防，为奇袭和劝降做准备。",effects:["情报↑","谋略人物受益"],max:1,run:s=>{const n=owns(s,"clocktower")?2:1;let found=0;for(let i=0;i<n;i++){const targets=attackableTerritories(s).filter(id=>!s.intel[id]);if(!targets.length)break;const id=pick(targets);s.intel[id]=true;found++;const p=postureOf(s,id);log(s,"good",`已摸清${TERRITORY_DEFS[id].name}的驻防${p?`——对方摆的是${p.name}（${p.hint}）`:"和主将"}。`)}if(!found){change(s,"heat",-4);log(s,"story","魏小楼的路子暂时没有新消息。")}}},
  ...
  {id:"fortify",icon:"守",name:"加固驻防",desc:"把一批人手常驻在最薄弱的地盘上，砌墙、看门、守夜。",effects:["该地驻防+18","能战-12"],max:1,canRun:s=>s.crew>=12&&ownTerritories(s).length>0,lockedText:s=>"能战人手不足12人",run:s=>{const id=weakestOwned(s);s.crew-=12;s.territories[id].guard+=18;change(s,"support",2);log(s,"good",`12 个人留在了${TERRITORY_DEFS[id].name}，驻防加厚到 ${s.territories[id].guard}。`)}},
...
function createInitialState(name="沈川",creed="yi",difficulty="standard",mutators=null){
  ...
  postures:{},governors:{},truces:{},incited:null,schedule:[],crisisCooldowns:{},aiPityUntil:0,mutators:Array.isArray(mutators)?mutators.filter(id=>MUTATORS[id]).slice(0,2):[]};
...
function monthlyReportModal(s,eco,rec,moves,harass,flavor){
  const lines=[];
  ...
  return{title:`第${s.month}月 · 雾港月报`,portrait:CHARACTER_DEFS.sumanqing.portrait,body:
    ...
    `<div class="report-block"><b>人手</b><p>${rec.back+rec.healed>0?`${rec.back} 人整补归队、${rec.healed} 人伤愈。`:"没有人员回流。"}当前能战 ${s.crew} · 整补 ${s.regroup} · 养伤 ${s.wounded}。</p></div>`+
    `<div class="report-block"><b>雾港动向</b>${(lines.length?lines:["这个月，对手们都异常安静——安静得反常。"]).map(x=>`<p>${esc(x)}</p>`).join("")}</div>`+
...
function advanceMonth(s,force=false,rng=Math.random){if(s.battleSession){toast("先把这场血拼打完");return false}if(s.ended)return false;if(s.ap>0&&!force){enqueue({title:"本月还有行动点",body:`<p>还剩 <b>${s.ap}</b> 个行动点。它们不会带到下个月。</p>`,options:[option("继续安排","回到议事堂",()=>{}),option("直接进入下月","放弃剩余行动点",()=>setTimeout(()=>advanceMonth(s,true),80),"danger")]},"时间确认");return false}
  ...
  const eco=applyEconomy(s);checkInsolvency(s);
  ...
  change(s,"morale",Math.round((58-s.morale)*.18));change(s,"heat",-2);refreshRecruitMarket(s,rng);enemyGrowth(s);
  ...
  const harass=enemyAttack(s,rng);
  ...
  modalQueue.unshift({...monthlyReportModal(s,eco,rec,moves,harass,flavor),kicker:"月度盘点"});
...
function renderMinimap(){
  ...
  const nodeSvg=Object.keys(TERRITORY_DEFS).map(id=>{
    ...
    const cls=`map-node${mine?" mine":""}${attackable.has(id)?" attackable":""}${mine&&t.settling>0?" settling":""}`;
...
if(typeof module!=="undefined"&&module.exports)module.exports={CHARACTER_DEFS,TERRITORY_DEFS,RANDOM_EVENTS,CHAIN_STEPS,POSTURES,MUTATORS,sortLeaderboard,rankOf,recordLeaderboard,loadLeaderboard,postureOf,postureMult,rerollPosture,rollMutators,governorTick,isGovernor,governorOf,truceCost,inciteChance,aliveAIFactions,checkCrises,checkChapter,scheduleIn,pumpSchedule,createInitialState,makeCommonCandidate,refreshRecruitMarket,hireCommon,totalCrew,crewCap,drainCrew,recoverCrew,officerTension,enemyTurn,enemyGrowth,effectiveGuard,tickSettling,settlingTerritories,woundedCareCost,monthlyGross,monthlyUpkeep,attackableTerritories,estimateBattle,startBattle,stageOptions,applyStageChoice,finishBattle,resolveBattle,advanceMonth,ownTerritories,officerCapacity,applyAction,applyEconomy,checkInsolvency,monthDisplay,normalizeState,namedCandidateStatus};
